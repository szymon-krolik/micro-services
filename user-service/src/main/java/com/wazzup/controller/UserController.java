...
public class UserController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success get user"),
            @ApiResponse(responseCode = "400", description = "User not found")
    })
    @ResponseStatus(HttpStatus.OK)
    @GetMapping("/by-token")
    public UserInformationWithIdDTO getUserByToken(HttpServletRequest request) {
        return USER_SERVICE.getUserInformationWithIdByEmail(request);
    }
